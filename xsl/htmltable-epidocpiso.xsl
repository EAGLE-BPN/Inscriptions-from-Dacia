    ...
    <xsl:template match="/">
        <xsl:for-each select="/HTML/BODY/TABLE/TR">
            <xsl:variable name="id">
                <xsl:value-of select="concat('ip',format-number(number(position()),'00000000'))"/>
            </xsl:variable>
...
<!--NO HD NUMBERS PROVIDED = NO TM APPLICABLE!! and NO valid URI provided!!-->
  <!--    <xsl:if test="TD[position()=32]/text()"><idno type="TM"><xsl:variable select="normalize-space(TD[position()=32])" name="tm"/><xsl:value-of select="document('https://raw.githubusercontent.com/EAGLE-BPN/epidocupconversion/master/allinone/edh-tm.htm')//td[preceding-sibling::td[lower-case(.) = lower-case($tm)]]"/></idno></xsl:if>
                -->
                <availability>
...
This file is licensed under the Creative Commons Attribution-NonCommercial-ShareAlike 3.0 Unported license.
            ...
            <sourceDesc>
                <msDesc>
                    <msIdentifier>
                        <country>
                            <placeName type="modern"><xsl:value-of select=" normalize-space(TD[position()=20])"/></placeName>
                        </country>
                        <region>
                            <placeName type="modern"><xsl:if test="normalize-space(TD[position()=21])"><xsl:value-of select="normalize-space(TD[position()=21])"/></xsl:if></placeName>
                        </region>
                        <settlement>
                            <placeName><xsl:value-of select="normalize-space(TD[position()=17])"/></placeName>                     
                        </settlement>
                        <repository><xsl:value-of select="normalize-space(TD[position()=19])"/></repository>
                        <collection/>
                        <idno/>
                    </msIdentifier>
                    <physDesc>
...
<objectType><xsl:variable name="noquestion"><xsl:analyze-string select="normalize-space(TD[position()=7])" regex="(\w+)\?"><xsl:matching-substring><xsl:value-of select="regex-group(1)"/><!--
--></xsl:matching-substring><xsl:non-matching-substring><xsl:analyze-string select="." regex="(\w+),\s(\w*)"><xsl:matching-substring><xsl:value-of select="regex-group(1)"/></xsl:matching-substring><!--            
                ...
                --><xsl:variable name="voc_term"><xsl:choose><!--
                    ...
                    --></xsl:otherwise></xsl:choose></xsl:variable><xsl:if test="$voc_term!=''"><xsl:attribute name="ref"><xsl:value-of select="$voc_term"/></xsl:attribute></xsl:if><xsl:value-of select="normalize-space(TD[position()=7])"/><!--
--></objectType>
                                    <material><xsl:variable name="noquestion"><xsl:analyze-string select="normalize-space(TD[position()=6])" regex="(\w+)\?"><xsl:matching-substring><xsl:value-of select="regex-group(1)"/><!--
--></xsl:matching-substring><xsl:non-matching-substring><xsl:analyze-string select="." regex="(\w+),\s(\w*)"><xsl:matching-substring><xsl:value-of select="regex-group(1)"/></xsl:matching-substring><!--            
                    --><xsl:non-matching-substring><xsl:value-of select="."/></xsl:non-matching-substring></xsl:analyze-string></xsl:non-matching-substring></xsl:analyze-string></xsl:variable><!--
                --><xsl:variable name="voc_term"><xsl:choose><!--
                        ...
                        --><xsl:value-of select="$seq[1]"/><!--this is not very clever but gives at least coherent results and one value only when vocabularies have many possible...--><!--
                    --></xsl:otherwise></xsl:choose></xsl:variable><xsl:if test="$voc_term!=''"><xsl:attribute name="ref"><xsl:value-of select="$voc_term"/></xsl:attribute></xsl:if><xsl:value-of select="normalize-space(TD[position()=6])"/><!--
--></material>                                    <p/>
                                    <dimensions unit="cm">
                                <xsl:if test="TD[position()=8]"><height><xsl:variable name="s" as="xs:string*"><xsl:analyze-string select="normalize-space(TD[position()=8])" regex="(\d+(.|,)?\d+?)"><xsl:matching-substring><xsl:sequence select="regex-group(1)"/></xsl:matching-substring></xsl:analyze-string></xsl:variable><!--<xsl:attribute name="atMost"><xsl:value-of select="$s[1]"/></xsl:attribute><xsl:if test="$s[2]"><xsl:attribute name="atLeast"><xsl:value-of select="$s[2]"/></xsl:attribute></xsl:if>--><xsl:value-of select="$s[1]"/><xsl:if test="$s[2]"><xsl:text>; </xsl:text><xsl:value-of select="$s[2]"/></xsl:if></height></xsl:if>
                                         <xsl:if test="TD[position()=9]"><width><xsl:variable name="s" as="xs:string*"><xsl:analyze-string select="normalize-space(TD[position()=9])" regex="(\d+(.|,)?\d+?)"><xsl:matching-substring><xsl:sequence select="regex-group(1)"/></xsl:matching-substring></xsl:analyze-string></xsl:variable><!--<xsl:attribute name="atMost"><xsl:value-of select="$s[1]"/></xsl:attribute><xsl:if test="$s[2]"><xsl:attribute name="atLeast"><xsl:value-of select="$s[2]"/></xsl:attribute></xsl:if>--><xsl:value-of select="$s[1]"/><xsl:if test="$s[2]"><xsl:text>; </xsl:text><xsl:value-of select="$s[2]"/></xsl:if></width></xsl:if>
                                         <xsl:if test="TD[position()=10]"><depth><xsl:variable name="s" as="xs:string*"><xsl:analyze-string select="normalize-space(TD[position()=10])" regex="(\d+(.|,)?\d+?)"><xsl:matching-substring><xsl:sequence select="regex-group(1)"/></xsl:matching-substring></xsl:analyze-string></xsl:variable><!--<xsl:attribute name="atMost"><xsl:value-of select="$s[1]"/></xsl:attribute><xsl:if test="$s[2]"><xsl:attribute name="atLeast"><xsl:value-of select="$s[2]"/></xsl:attribute></xsl:if>--><xsl:value-of select="$s[1]"/><xsl:if test="$s[2]"><xsl:text>; </xsl:text><xsl:value-of select="$s[2]"/></xsl:if></depth></xsl:if>
                                            </dimensions>
                                                                        <rs type="decoration"><xsl:variable name="noquestion"><xsl:analyze-string select="normalize-space(TD[position()=11])" regex="(\w+)\?"><xsl:matching-substring><xsl:value-of select="regex-group(1)"/><!--
--></xsl:matching-substring><xsl:non-matching-substring><xsl:analyze-string select="." regex="(\w+),\s(\w*)"><xsl:matching-substring><xsl:value-of select="regex-group(1)"/></xsl:matching-substring><!--            
                    --><xsl:non-matching-substring><xsl:value-of select="."/></xsl:non-matching-substring></xsl:analyze-string></xsl:non-matching-substring></xsl:analyze-string></xsl:variable><!--
                --><xsl:variable name="voc_term"><xsl:choose><!--
...
--></rs>
                                    <rs type="statPreserv"><xsl:variable name="noquestion"><xsl:analyze-string select="normalize-space(TD[position()=34])" regex="(\w+)\?"><xsl:matching-substring><xsl:value-of select="regex-group(1)"/><!--
--></xsl:matching-substring><xsl:non-matching-substring><xsl:analyze-string select="." regex="(\w+),\s(\w*)"><xsl:matching-substring><xsl:value-of select="regex-group(1)"/></xsl:matching-substring><!--            
                    --><xsl:non-matching-substring><xsl:value-of select="."/></xsl:non-matching-substring></xsl:analyze-string></xsl:non-matching-substring></xsl:analyze-string></xsl:variable><!--
                --><xsl:variable name="voc_term"><xsl:choose><!--
                        ...
                        --><xsl:value-of select="$seq[1]"/><!--this is not very clever but gives at least coherent results and one value only when vocabularies have many possible...--><!--
                    --></xsl:otherwise></xsl:choose></xsl:variable><xsl:if test="$voc_term!=''"><xsl:attribute name="ref"><xsl:value-of select="$voc_term"/></xsl:attribute></xsl:if><xsl:value-of select="normalize-space(td[position()=34])"/><!--
--></rs> </support>
                            </supportDesc>
                            <layoutDesc>
                                <layout>
                                   <rs type="execution"><xsl:variable name="noquestion"><xsl:analyze-string select="normalize-space(TD[position()=31])" regex="(\w+)\?"><xsl:matching-substring><xsl:value-of select="regex-group(1)"/><!--
--></xsl:matching-substring><xsl:non-matching-substring><xsl:analyze-string select="." regex="(\w+),\s(\w*)"><xsl:matching-substring><xsl:value-of select="regex-group(1)"/></xsl:matching-substring><!--            
                    --><xsl:non-matching-substring><xsl:value-of select="."/></xsl:non-matching-substring></xsl:analyze-string></xsl:non-matching-substring></xsl:analyze-string></xsl:variable><!--
                --><xsl:variable name="voc_term"><xsl:choose><!--
                        ...
                        --><xsl:value-of select="$seq[1]"/><!--this is not very clever but gives at least coherent results and one value only when vocabularies have many possible...--><!--
                    --></xsl:otherwise></xsl:choose></xsl:variable><xsl:if test="$voc_term!=''"><xsl:attribute name="ref"><xsl:value-of select="$voc_term"/></xsl:attribute></xsl:if><xsl:value-of select="normalize-space(TD[position()=31])"/><!--
--><!--<xsl:text>; </xsl:text><xsl:value-of select="TD[position()=13]"/>--></rs> <dimensions>
                                        <width unit="cm"/>
                                        <height unit="cm"/>
                                    </dimensions>
                        ...
                        <handDesc>
                            <handNote>
                               <xsl:value-of select="normalize-space(TD[position()=14])"/>
                                <height unit="cm"><xsl:analyze-string select="normalize-space(TD[position()=13])" regex="(\d+(.|,)?\d+?)"><xsl:matching-substring><xsl:value-of select="regex-group(1)"/></xsl:matching-substring></xsl:analyze-string></height>
                            </handNote>
                        </handDesc>
                    </physDesc>
                    <history>
                        <origin>
                            <origPlace><xsl:if test="TD[position()=30]"><xsl:value-of select="normalize-space(TD[position()=30])"/></xsl:if>
                                <placeName><xsl:for-each select="normalize-space(TD[position()=15])"><xsl:variable name="noquestion"><xsl:analyze-string select="." regex="(\w+)\?"><xsl:matching-substring><!--
                    ...
                    --><xsl:variable name="seq" select="document('../../Dropbox/Heidelberg/EAGLE/CONTENT_PREPARATION/UPConversion/epidocupconversion/allinone/TMGeoIDToponyms.XML')//f:RESULTSET/f:ROW/f:COL[2]/f:DATA[contains(lower-case(.), lower-case($noquestion))]/parent::f:COL/preceding-sibling::f:COL/f:DATA"/><!--
                        ...
                        --><xsl:value-of select="$seq[1]"/></xsl:otherwise></xsl:choose></xsl:variable><xsl:attribute name="ref"><xsl:value-of select="concat('www.trismegistos.org/place/',format-number(number($voc_term),'000000'))"/></xsl:attribute><xsl:value-of select="."/></xsl:for-each><!--<xsl:value-of select="normalize-space(td[position()=18])"/>--></placeName>
                                <placeName type="provincItalicRegion"><xsl:for-each select="normalize-space(TD[position()=16])"><xsl:variable name="noquestion"><xsl:analyze-string select="." regex="(\w+)\?"><xsl:matching-substring><!--
                        --><xsl:value-of select="regex-group(1)"/></xsl:matching-substring><xsl:non-matching-substring><xsl:analyze-string select="." regex="((\w+)\s*\w*)(,|:)\s((\w*\*?)\s*\w*\s*\w*)"><!--
                            --><xsl:matching-substring><xsl:value-of select="regex-group(1)"/><!--  if the toponym is before the , or : needs to be regex-group(1) if it is after, regex-group(4) --></xsl:matching-substring><xsl:non-matching-substring><!--
                                --><xsl:value-of select="."/></xsl:non-matching-substring></xsl:analyze-string></xsl:non-matching-substring></xsl:analyze-string></xsl:variable><xsl:variable name="voc_term"><!--
                                        --><xsl:choose><xsl:when test="document('../../Dropbox/Heidelberg/EAGLE/CONTENT_PREPARATION/UPConversion/epidocupconversion/allinone/TMGeoIDToponyms.XML')//f:RESULTSET/f:ROW/f:COL[2]/f:DATA[contains(lower-case(.), lower-case($noquestion))]"><!--
                    --><xsl:variable name="seq" select="document('../../Dropbox/Heidelberg/EAGLE/CONTENT_PREPARATION/UPConversion/epidocupconversion/allinone/TMGeoIDToponyms.XML')//f:RESULTSET/f:ROW/f:COL[2]/f:DATA[contains(lower-case(.), lower-case($noquestion))]/parent::f:COL/preceding-sibling::f:COL/f:DATA"/><!--
                        ...
                        --><xsl:value-of select="$seq[1]"/></xsl:otherwise></xsl:choose></xsl:variable><xsl:attribute name="ref"><xsl:value-of select="concat('www.trismegistos.org/place/',format-number(number($voc_term),'000000'))"/></xsl:attribute><xsl:value-of select="."/></xsl:for-each></placeName>
                              ...
                              <origDate><xsl:if test="normalize-space(TD[position()=32])"><xsl:attribute name="notBefore-custom"><xsl:value-of select="format-number(number(TD[position()=32]), '0000')"/></xsl:attribute></xsl:if><xsl:if test="normalize-space(TD[position()=33])"><xsl:attribute name="notAfter-custom"><xsl:value-of select="format-number(number(TD[position()=33]), '0000')"/></xsl:attribute></xsl:if><xsl:attribute name="datingMethod">http://en.wikipedia.org/wiki/Julian_calendar</xsl:attribute><xsl:value-of select="normalize-space(TD[position()=29])"/></origDate>
                        </origin>
                        <provenance type="found">  
                            <date><xsl:value-of select="normalize-space(TD[position()=18])"/></date>
                                <placeName><xsl:value-of select="normalize-space(TD[position()=17])"/></placeName> 
                            <placeName type="modernRegion"><xsl:if test="normalize-space(TD[position()=21])"><xsl:value-of select="normalize-space(TD[position()=21])"/></xsl:if></placeName>
                            <placeName type="modernCountry"><xsl:value-of select="normalize-space(TD[position()=20])"/></placeName>
                        </provenance>
                    </history>
                </msDesc>
            </sourceDesc>
        ...
        <profileDesc>
                <textClass>
<keywords>
    <term><xsl:variable name="noquestion"><xsl:analyze-string select="normalize-space(TD[position()=5])" regex="(\w+)\?"><xsl:matching-substring><xsl:value-of select="regex-group(1)"/><!--
--></xsl:matching-substring><xsl:non-matching-substring><xsl:analyze-string select="." regex="(\w+),\s(\w*)"><xsl:matching-substring><xsl:value-of select="regex-group(1)"/></xsl:matching-substring><!--            
                    --><xsl:non-matching-substring><xsl:value-of select="."/></xsl:non-matching-substring></xsl:analyze-string></xsl:non-matching-substring></xsl:analyze-string></xsl:variable><!--
                --><xsl:variable name="voc_term"><xsl:choose><!--
                        ...
                        --><xsl:value-of select="$seq[1]"/><!--this is not very clever but gives at least coherent results and one value only when vocabularies have many possible...--><!--
                    --></xsl:otherwise></xsl:choose></xsl:variable><xsl:if test="$voc_term!=''"><xsl:attribute name="ref"><xsl:value-of select="$voc_term"/></xsl:attribute></xsl:if><xsl:value-of select="normalize-space(TD[position()=5])"/><!--
--></term>
</keywords>
    </textClass>
        ...
        <revisionDesc>
         <change><xsl:attribute name="when"><xsl:value-of select="format-date(current-date(), '[Y]-[M, 2]-[D, 2]')"/></xsl:attribute><!--
             --><xsl:attribute name="who">IoanPiso</xsl:attribute><xsl:text>Ioan Piso</xsl:text></change>
        </revisionDesc>
    </teiHeader>
        <xsl:if test="normalize-space(TD[position()=3])"> <facsimile>
<xsl:for-each select="normalize-space(TD[position()=3])"><xsl:choose><xsl:when test="contains(.,';')"><xsl:for-each select="tokenize(.,';')">
    <graphic><xsl:attribute name="n"><xsl:value-of select="."/></xsl:attribute><xsl:attribute name="url"><xsl:value-of select="concat('https://commons.wikimedia.org/wiki/File:',.,'.jpg')"/></xsl:attribute>
    ...
    <text>
        <body>
            ...
            <div type="bibliography">
                <listBibl>
<xsl:for-each select="normalize-space(TD[position()=4])"><xsl:choose><xsl:when test="contains(.,';')"><xsl:for-each select="tokenize(.,';')"><bibl><xsl:value-of select="."/></bibl></xsl:for-each></xsl:when><xsl:otherwise><bibl><xsl:value-of select="."/></bibl></xsl:otherwise></xsl:choose></xsl:for-each>
                </listBibl>
            </div>
            
            
            <div type="edition"><xsl:attribute name="xml:lang"><xsl:choose><xsl:when test="TD[position()=22] = 'latin'">la</xsl:when><!--
    --><xsl:when test="TD[position()=22] = 'greek'">grc</xsl:when><xsl:otherwise>la</xsl:otherwise></xsl:choose></xsl:attribute><!--
                --><head>Text</head><xsl:variable name="textepidoc">  
            <xsl:choose>
                <xsl:when test="contains(normalize-space(TD[position()=23]),'//')">
                    <xsl:for-each select="tokenize(normalize-space(TD[position()=23]),'//')">
                ...
                <xsl:otherwise>
<xsl:for-each select="normalize-space(TD[position()=23])">
    <ab>
                        <lb/>
                        <xsl:variable name="brackets">
                            <xsl:call-template name="breakbrackets">
...
<xsl:if test="normalize-space(TD[position()=24])">
            <div type="apparatus">
                <p><xsl:value-of select="normalize-space(TD[position()=24])"/></p>
            </div>
</xsl:if>
...
<xsl:if test="normalize-space(TD[position()=25])">
            <div type="commentary">
                <p><xsl:value-of select="normalize-space(TD[position()=25])"/></p>
            </div>
</xsl:if>
        </body>
    </text>
        ...
        <xsl:result-document href="../xml/indexpiso.xml" format="xml">
        ...
        <xsl:result-document href="../html/indexpiso.html" format="html">
                            ...
                            <xsl:for-each select="/HTML/BODY/TABLE/TR">
                                ...
                                <li>
                                    <a href="{concat('text/',$id)}">
                                        <xsl:choose><xsl:when test="TD[position()=2]/text()"><xsl:value-of select="TD[position()=2]"/></xsl:when><xsl:otherwise>
                                            <xsl:value-of select="TD[position()=7]"/></xsl:otherwise></xsl:choose><xsl:text> (</xsl:text><xsl:value-of select="$id"/><xsl:text>)</xsl:text>
                                    </a>
                                </li>
