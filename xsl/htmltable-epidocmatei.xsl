...
<!--    
    ...
    <xsl:template match="/">
        <xsl:for-each select="/HTML/BODY/TABLE/TR">
            <xsl:variable name="id">
                <xsl:value-of select="concat('fm',format-number(number(position()),'00000000'))"/>
            </xsl:variable>
    ...
    <teiHeader>
        <fileDesc>
            <titleStmt>
                <title><xsl:value-of select="normalize-space(TD[position()=11])"/><xsl:text> on </xsl:text><xsl:value-of select="normalize-space(TD[position()=14])"/></title>
            </titleStmt>  
            <publicationStmt>
...
<!--NO HD NUMBERS PROVIDED = NO TM APPLICABLE!! and NO valid URI provided!!-->
  <!--    <xsl:if test="TD[position()=32]/text()"><idno type="TM"><xsl:variable select="normalize-space(TD[position()=32])" name="tm"/><xsl:value-of select="document('../../Dropbox/Heidelberg/EAGLE/CONTENT_PREPARATION/UPConversion/epidocupconversion/allinone/edh-tm.htm')//td[preceding-sibling::td[lower-case(.) = lower-case($tm)]]"/></idno></xsl:if>
...
--><!--<xsl:text>; </xsl:text><xsl:value-of select="TD[position()=13]"/>--></rs><dimensions>
                    ...
                    --><xsl:variable name="seq" select="document('../../Dropbox/Heidelberg/EAGLE/CONTENT_PREPARATION/UPConversion/epidocupconversion/allinone/TMGeoIDToponyms.XML')//f:RESULTSET/f:ROW/f:COL[2]/f:DATA[contains(lower-case(.), lower-case($noquestion))]/parent::f:COL/preceding-sibling::f:COL/f:DATA"/><!--
                        ...
                        --><xsl:value-of select="$seq[1]"/></xsl:otherwise></xsl:choose></xsl:variable><xsl:attribute name="ref"><xsl:value-of select="concat('www.trismegistos.org/place/',format-number(number($voc_term),'000000'))"/></xsl:attribute><xsl:value-of select="."/></xsl:for-each><!--<xsl:value-of select="normalize-space(td[position()=18])"/>--></placeName>
                                <placeName type="provincItalicRegion"><xsl:for-each select="normalize-space(TD[position()=32])"><xsl:variable name="noquestion"><xsl:analyze-string select="." regex="(\w+)\?"><xsl:matching-substring><!--
                        --><xsl:value-of select="regex-group(1)"/></xsl:matching-substring><xsl:non-matching-substring><xsl:analyze-string select="." regex="((\w+)\s*\w*)(,|:)\s((\w*\*?)\s*\w*\s*\w*)"><!--
                            --><xsl:matching-substring><xsl:value-of select="regex-group(1)"/><!--  if the toponym is before the , or : needs to be regex-group(1) if it is after, regex-group(4) --></xsl:matching-substring><xsl:non-matching-substring><!--
                                --><xsl:value-of select="."/></xsl:non-matching-substring></xsl:analyze-string></xsl:non-matching-substring></xsl:analyze-string></xsl:variable><xsl:variable name="voc_term"><!--
                                        --><xsl:choose><xsl:when test="document('../../Dropbox/Heidelberg/EAGLE/CONTENT_PREPARATION/UPConversion/epidocupconversion/allinone/TMGeoIDToponyms.XML')//f:RESULTSET/f:ROW/f:COL[2]/f:DATA[contains(lower-case(.), lower-case($noquestion))]"><!--
...
--></term></keywords>
        ...
        <revisionDesc>
         <change><xsl:attribute name="when"><xsl:value-of select="format-date(current-date(), '[Y]-[M, 2]-[D, 2]')"/></xsl:attribute><!--
             --><xsl:attribute name="who">FlorianMatei-Popescu</xsl:attribute><xsl:text>Florian Matei-Popescu</xsl:text></change>
        </revisionDesc>
    </teiHeader>
        <xsl:if test="normalize-space(TD[position()=2])"> <facsimile>
<xsl:for-each select="normalize-space(TD[position()=2])"><xsl:choose><xsl:when test="contains(.,';')"><xsl:for-each select="tokenize(.,';')">
    <graphic><xsl:attribute name="n"><xsl:value-of select="."/></xsl:attribute><xsl:attribute name="url"><xsl:value-of select="concat('https://commons.wikimedia.org/wiki/File:',normalize-space(.),'.jpg')"/></xsl:attribute>
    ...
    <text>
        <body>
            ...
            <div type="bibliography">
                <listBibl>
<xsl:for-each select="normalize-space(TD[position()=26])"><xsl:choose><xsl:when test="contains(.,';')"><xsl:for-each select="tokenize(.,';')"><bibl><xsl:value-of select="."/></bibl></xsl:for-each></xsl:when><xsl:when test="contains(.,'=')"><xsl:for-each select="tokenize(.,'=')"><bibl><xsl:value-of select="."/></bibl></xsl:for-each></xsl:when><xsl:otherwise><bibl><xsl:value-of select="."/></bibl></xsl:otherwise></xsl:choose></xsl:for-each>
                </listBibl>
            </div>
            
            
            <div type="edition" xml:lang="la"><xsl:attribute name="xml:lang"><xsl:choose><xsl:when test="TD[position()=12] = 'latin'">la</xsl:when><!--
    --><xsl:when test="TD[position()=12] = 'greek'">grc</xsl:when><xsl:otherwise>la</xsl:otherwise></xsl:choose></xsl:attribute><!--
...
<xsl:if test="normalize-space(TD[position()=27])">
            <div type="commentary">
                <p><xsl:value-of select="normalize-space(TD[position()=27])"/></p>
            </div>
</xsl:if>
        </body>
    </text>
        ...
        <xsl:result-document href="../xml/indexmatei.xml" format="xml">
        ...
        <xsl:result-document href="../html/indexmatei.html" format="html">
